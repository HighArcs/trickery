...
pub fn activity_two() {
    ...
    writeln!(f, "Enter the first x-coordinate:");
    let x1 = s.next_double();

    writeln!(f, "Enter the second x-coordinate:");
    let x2 = s.next_double();

    writeln!(f, "Enter the first y-coordinate:");
    let y1 = s.next_double();

    writeln!(f, "Enter the second y-coordinate:");
    ...
    writeln!(f, "Slope: {slope}")
}

pub fn activity_three() {
    let s = Scanner::new();

    writeln!(f, "Enter two doubles:");
    let x = s.next_double();
    let y = s.next_double();

    let d = (x - y).abs().round() as i32;

    writeln!(f, "Difference: {d}");
}
